...
parse_check <- function(user_code) {
  ...
  marked_code <- ""
  correct <- TRUE
  # look for blanks as identified by pairs  of <<
  blanks_found <- regmatches(user_code, regexpr("(<<[^> =,\\)]*>>)", user_code))
  if (length(blanks_found) > 0) {
    message <- paste("There is an unfilled-in blank:",
                     paste(blanks_found, collapse = ", "))
